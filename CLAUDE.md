...
4. `yt-script` **발행 모드** (`/yt-push` → `yt-script --publish <summary_md> <topic>`): 저장된 요약 .md 를 발행 루트 `YT_PUBLISH_DIR`(외부 git repo 안 폴더)의 `{topic}/` 로 복사 후 `pull --rebase`→`add`·`commit`·`push`. **핵심: 발행 위치는 "유튜브 출처"가 아니라 "내용 주제"로 정한다** — `/yt-push` 가 요약 내용 + `yt-script --topics`(기존 폴더 목록)를 보고 **재사용/신설을 판단**해 `topic` 을 넘긴다(예: Claude Code 영상 → `claude-code/`). topic 은 `--save` 의 도메인과 같은 규칙으로 정규화. `--publish` 에서 summary 생략 시 최근 .md, topic 생략 시 로컬 도메인명으로 폴백(터미널 편의). 보조: `--latest`(최근 요약본 경로, `LATEST_FILE:`), `--topics`(`PUBLISH_ROOT:`+`TOPICS:`) 둘 다 부작용 없음. 성공 `PUSHED_FILE:`, 동일내용 `변경 없음`, push 실패 시 커밋만 남기고 `COMMITTED_FILE:`+경고. 추출/요약과 독립(`YT_PUBLISH_DIR` 없으면 발행만 비활성).
...
yt-script --publish <summary_md> <topic>    # 그 주제 폴더로 발행(보통은 /yt-push 가 topic 을 판단해 호출)
```
...
- `yt-push.md` `allowed-tools` 는 `Bash(yt-script:*), Read`(내용 파악용 Read + 발행은 `yt-script` 가 git 실행 → Write/Bash(git) 불필요). `--latest`/`--topics` 마커(`LATEST_FILE:`/`PUBLISH_ROOT:`/`TOPICS:`)를 바꾸면 yt-push.md 가 깨진다.
