...
## 깃 repo 로 발행 (선택)
요약본을 로컬에 저장하는 것에 더해, **원할 때 커맨드 한 번으로** 개인 git repo(공부 노트 등)에 올릴 수 있다.
이때 **분류 기준은 "유튜브에서 왔다"가 아니라 "무슨 내용이냐"** 다 — Claude 가 요약 내용과 repo 의 기존
폴더 구조를 보고 **알맞은 주제 폴더에 적재**한다. 관련 폴더가 있으면 그걸 확장(재사용)하고, 없으면 새로 만든다.
예: Claude Code 영상 요약은 같은 repo 의 `claude-code/` 폴더로(이미 있으면 거기에).

먼저 발행 대상 repo 를 clone 하고, **주제 폴더들이 들어갈 "발행 루트"** 를 `YT_PUBLISH_DIR` 로 지정한다(한 번만):
```bash
# 예: 개인 노트 repo 의 study 폴더를 발행 루트로
export YT_PUBLISH_DIR="$HOME/Desktop/dev-lab/study"   # ~/.zshrc 에 추가
```
그다음 발행은 **Claude Code 에서 `/yt-push`** 로 한다(인자 없으면 가장 최근 요약본 1건):
```
/yt-push                       # 최근 요약본을 내용에 맞는 주제 폴더로 발행
